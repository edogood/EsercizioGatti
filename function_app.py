...
def vote_cat(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not (1 <= vote <= 5):
        return func.HttpResponse(
            json.dumps({"message": "Vote must be between 1 and 5."}), status_code=400
        )
    with get_db_connection() as conn, conn.cursor() as cursor:
        cursor.execute("INSERT INTO Votes (CatID, Vote) VALUES (?, ?)", (cat_id, vote))
        conn.commit()
    return func.HttpResponse(
        json.dumps({"message": "Vote recorded successfully!"}), status_code=201
    )
...
def get_cats(req: func.HttpRequest) -> func.HttpResponse:
    with get_db_connection() as conn, conn.cursor() as cursor:
        ...
        rows = cursor.fetchall()
        result = [{"CatID": row[0], "Name": row[1], "ImageURL": row[2]} for row in rows]
    return func.HttpResponse(json.dumps(result), status_code=200)
